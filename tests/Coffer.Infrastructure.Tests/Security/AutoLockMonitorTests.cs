...
public class AutoLockMonitorTests
{
    ...
    public async Task Start_AfterAutoLockTriggered_StopsItself()
    {
        ...
        monitor.Start(TimeSpan.FromMilliseconds(50));
        InvokeOnTick(monitor); // raises once
        InvokeOnTick(monitor); // monitor is internally stopped — must not raise again
        await Task.Delay(20);
    ...
    private static void InvokeOnTick(AutoLockMonitor monitor)
    {
        // The Timer's 60-second cadence is fine for production but unobservable in
        // tests; reach into the private callback directly to assert the policy.
