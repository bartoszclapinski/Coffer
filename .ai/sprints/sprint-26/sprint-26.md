...
- [ ] 26.1 `IMasterPasswordService` (`Coffer.Core/Security/`): `Task ChangeMasterPasswordAsync(string currentPassword, string newPassword, CancellationToken ct)`. Documents the thrown types (`InvalidMasterPasswordException`, `VaultMissingException`, `VaultCorruptedException`).
- [ ] 26.2 `MasterPasswordService` (`Coffer.Infrastructure/Security/`, `IMasterKeyDerivation` + `IKeyVault` + `IVaultPaths` + `ILogger`): read `DekFile` (missing → `VaultMissingException`; malformed → `VaultCorruptedException`); derive the old master key from `currentPassword` + the file's salt/params; `AesGcmCrypto.Decrypt` the password blob (`CryptographicException` → `InvalidMasterPasswordException`) — this both verifies the current password and recovers the DEK; derive a new master key (fresh salt, `Argon2Parameters.Default`); re-wrap the DEK; rewrite the file preserving `file.Version` + the seed blob via `DekFile.WriteReplaceAsync`; refresh the DPAPI cache with the new key. Zero every key buffer in `finally`.
- [ ] 26.3 DI: register `IMasterPasswordService` → `MasterPasswordService` in `AddCofferLogin` (transient, alongside `ISeedRecoveryService`).
- [ ] 26.4 Tests (`Coffer.Infrastructure.Tests/Security`): on a v2 vault, the right current password rotates so the **new** password logs in (`LoginService`) and the **old** one throws `InvalidMasterPasswordException`; a wrong current password throws and leaves the file byte-identical (no change); after a v2 rotation the **seed still recovers** the DEK (seed blob preserved) and the file is still v2; a v1 vault rotates and stays v1; the DPAPI cache holds a key that decrypts the rewritten blob; no password/key is logged. `TestVaultPaths` + the existing DEK-file test helpers.
