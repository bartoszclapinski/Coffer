...
- [x] 25.1 `DekFile` v2: added nullable seed-wrap fields (`byte[]? SeedIv, SeedTag, SeedCiphertext`) + `HasSeedWrap`; `WriteAsync` emits `version=2` + the seed blob after the password blob (and throws if a v2 record lacks the seed blob); `ReadAsync` accepts `version is 1 or 2`, reading the seed blob only for v2 (null for v1). `CurrentVersion = 2`, `PasswordOnlyVersion = 1`. Round-trip + truncation guards kept.
- [x] 25.2 Seed-derivation constant: `VaultSeedDerivation.Passphrase = "Coffer"` (internal) used by every wrap/unwrap, so setup, enable, and restore agree.
- [x] 25.3 `SetupService` writes v2: injects `ISeedManager`; after generating the DEK, also derives the recovery key from the mnemonic and `AesGcmCrypto.Encrypt`s the DEK with it; writes both blobs in the v2 `DekFile`; zeroes the recovery key + seed-encryption buffers. Rollback unchanged.
- [x] 25.4 `ISeedRecoveryService` (`Coffer.Core/Security/`) + `SeedRecoveryService` (`Coffer.Infrastructure/Security/`): `RecoverWithSeedAsync`, `EnableSeedRecoveryAsync`, `IsSeedRecoveryEnabledAsync`. Injects `IMasterKeyDerivation`, `ISeedManager`, `IKeyVault`, `IDekHolder`, `IVaultPaths`, `ILogger`. Recover: v1 → `SeedRecoveryUnavailableException`; wrong seed → `InvalidRecoverySeedException`; else decrypt via seed blob, re-wrap under a new master key (fresh salt), rewrite v2 atomically (same seed blob), publish + cache. Enable: requires `IDekHolder.IsAvailable`; keeps the password blob, adds the seed blob, rewrites v2. All key buffers zeroed in `finally`.
- [x] 25.5 Typed exceptions (`Coffer.Core/Security/`): `SeedRecoveryUnavailableException` (v1 / no seed blob), `InvalidRecoverySeedException` (seed does not unlock). Neither carries seed/key detail.
- [x] 25.6 Atomic DEK rewrite: `DekFile.WriteReplaceAsync` (temp file + `File.Move(overwrite)`) so a crash mid-rewrite never destroys the only key file; setup's first write stays `CreateNew` (`WriteAsync`).
- [x] 25.7 DI: `ISeedRecoveryService` → `SeedRecoveryService` registered in `AddCofferLogin` (transient, like `LoginService`).
- [x] 25.8 Tests (`Coffer.Infrastructure.Tests/Security`, +11): `DekFile` v1 reads back (null seed fields), v2 round-trips, a v2 record without a seed blob refuses to write, `WriteReplaceAsync` overwrites; `SetupService` still writes cleanly (now v2); `RecoverWithSeedAsync` — right seed re-wraps under a new password (new logs in via `LoginService`, old fails, seed still enabled), wrong seed → `InvalidRecoverySeedException` (nothing published), v1 vault → `SeedRecoveryUnavailableException`, no vault → `VaultMissingException`; `EnableSeedRecoveryAsync` upgrades v1→v2 (the seed then recovers the same DEK) and requires a logged-in DEK; `IsSeedRecoveryEnabledAsync` reflects no-vault/v1/v2. Existing `LoginServiceTests` now cover a legacy v1 vault (back-compat). `TestVaultPaths` temp dirs.
